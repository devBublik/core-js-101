...
function extractNameFromTemplate(value) {
  ...
  return arr[1].trim().slice(0, arr[1].length - 2);
}
...
function getRectangleString(width, height) {
  ...
  const leftside = `│${' '.repeat(width - 2)}│\n`;
  const rightside = `└${'─'.repeat(width - 2)}┘\n`;
  return top + leftside.repeat(height - 2) + rightside;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (letter) => cipher[origin.indexOf(letter)]);
}
